...
public class ThreadPoolImpl implements IThreadPool {
    ...
    static List<ThreadPoolTest.Task> taskQueue = new LinkedList<ThreadPoolTest.Task>();
    //线程工作组
    ...
    @Override
    public void execute(ThreadPoolTest.Task task) {
    ...
    @Override
    public void execute(ThreadPoolTest.Task[] tasks) {
        synchronized (taskQueue) {
            for (ThreadPoolTest.Task task : tasks) {
                taskQueue.add(task);
            }
            taskQueue.notifyAll();
        }
    ...
    @Override
    public void execute(List<ThreadPoolTest.Task> tasks) {
        synchronized (taskQueue) {
            for (ThreadPoolTest.Task task : tasks) {
                taskQueue.add(task);
            }
    ...
    class WorkerThread extends Thread {
        ...
        @Override
        public void run() {
            ThreadPoolTest.Task runnable = null;
            //死循环
            while (isRunning) {
                //非线程安全，所以采用同步锁
                synchronized (taskQueue) {
